...
public class PlayerController2 : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        layerMask = LayerMask.GetMask(new string[] { "Stage", "Treasure", "Player1" });
    }

    // Update is called once per frame
    void Update()
    {
        if (Input.GetButtonDown("Fire4_2"))
        {
            ++colorNumber;
        }
        ...
        if (input == true)
        {
            directionY = Input.GetAxisRaw("Horizontal2");
            if (directionY > 0)
            {
                directionY = 1;
            }
        ...
        if (input == true)
        {
            directionZ = Input.GetAxisRaw("Vertical2");
            if (directionZ > 0)
            {
                directionZ = 1;
            }
        ...
        if (directionZ != 0 && !isRotate && !isMove)
        {
            ...
            {
                PlayerLimit.Instance.PlusLimit2();

                if (PlayerLimit.Instance.limit2 == false)
                {
        ...
        // 四角ボタンが押された　&&　方向転換中でない　&&　移動中でない場合、上下移動する
        if (input == true)
        {
            if (Input.GetButtonDown("Fire2_2") && !isRotate && !isMove)
            {
                // 地上形態
                if (!isFlying)
                {
        ...
        if (slow)
        {
            ...
            if (DFlag2 == false)
            {
                ...
                pDebuff2 = Instantiate(p_DebuffEfct);
                pDebuff2.name = "Debf_Efct02";
                pDebuff2.transform.position = transform.position;
            }
